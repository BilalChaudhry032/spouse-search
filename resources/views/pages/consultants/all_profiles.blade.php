                        ...
                        <div class="box">
                            ...
                            <div class="box-body">
                                <table class="table table-bordered table-hover" id="example2">
                                    ...
                                    @foreach($users as $u)
                                        <tr>
                                            <td>@if($u->profile_pic == 'default.png') <img
                                                        src="{{asset('../storage/uploads/avatar/default.png')}}"
                                                        alt="{{ $u->name }}" width="50px"
                                                        style="margin-right: 1rem"> @else <img
                                                        src="{{ $u->profile_pic  }}" alt="{{ $u->name }}" width="50px"
                                                        style="margin-right: 1rem">@endif {{ $u->name  }}</td>
                                            ...
                                            <td>{{ $u->cityy['city_name'] }}/{{ $u->state['name'] }}
                                                /{{ $u->countryy['country_name'] }}</td>

                                            <td>
                                                <a href="{{ url('consultant/profile',$u->id) }}"
                                                   class="label label-success" style="margin-right: 1rem">View </a>
                                                @if($u->consultant_id == Auth::user()->id)
                                                    <a href="{{ url('consultant/edit-profile',$u->id) }}"
                                                       class="label label-info" style="margin-right: 1rem">Edit</a>
                                                    <a href="{{ url('consultant/delete-profile',$u->id) }}"
                                                       class="label label-danger">Delete</a>
                                                @endif
                                            </td>
                                            <td>{{ $u->interested->count() }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
